...
type DisplayLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Display, err error)
...
// List lists all Displays in the indexer.
func (s *displayLister) List(selector labels.Selector) (ret []*v2.Display, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Display))
	})
	return ret, err
}
...
type DisplayNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.Display, error)
...
// List lists all Displays in the indexer for a given namespace.
func (s displayNamespaceLister) List(selector labels.Selector) (ret []*v2.Display, err error) {
...
// Get retrieves the Display from the indexer for a given namespace and name.
func (s displayNamespaceLister) Get(name string) (*v2.Display, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("display"), name)
	}
	return obj.(*v2.Display), nil
}
