...
type KnownNodesLister interface {
	...
	List(selector labels.Selector) (ret []*v1.KnownNodes, err error)
...
// List lists all KnownNodeses in the indexer.
func (s *knownNodesLister) List(selector labels.Selector) (ret []*v1.KnownNodes, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.KnownNodes))
	})
	return ret, err
}
...
type KnownNodesNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.KnownNodes, error)
...
// List lists all KnownNodeses in the indexer for a given namespace.
func (s knownNodesNamespaceLister) List(selector labels.Selector) (ret []*v1.KnownNodes, err error) {
...
// Get retrieves the KnownNodes from the indexer for a given namespace and name.
func (s knownNodesNamespaceLister) Get(name string) (*v1.KnownNodes, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("knownnodes"), name)
	}
	return obj.(*v1.KnownNodes), nil
}
